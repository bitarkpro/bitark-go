...
import (
	...
	"github.com/bitarkpro/bitark-go/chain/crypto/ss58"
	"github.com/bitarkpro/bitark-go/chain/model"
...
*/
func (c *Client) GetBlockByNumber(height int64) (*model.BlockResponse, error) {
...
*/
func (c *Client) GetBlockByHash(blockHash string) (*model.BlockResponse, error) {
	var (
	...
	blockResp := new(model.BlockResponse)
...
解析外部交易extrinsic
*/
func (c *Client) parseExtrinsicByDecode(extrinsics []string, blockResp *model.BlockResponse) error {
	var (
	...
	defer func() {
		if err := recover(); err != nil {
			...
			blockResp.Extrinsic = []*model.ExtrinsicResponse{}
	...
	for i, extrinsic := range extrinsics {
		...
		ed, err := chain.NewExtrinsicDecoder(c.Meta)
		if err != nil {
		...
		var resp model.ExtrinsicDecodeResponse
				...
				for _, param := range resp.Params {
					if param.Name == "calls" {
						...
						case []interface{}:
							...
							var values []model.UtilityParamsValue
	...
	blockResp.Extrinsic = make([]*model.ExtrinsicResponse, len(params))
	for idx, param := range params {
		e := new(model.ExtrinsicResponse)
...
解析当前区块的System.event
*/
func (c *Client) parseExtrinsicByStorage(blockHash string, blockResp *model.BlockResponse) error {
	var (
	...
	*/
	...
	//解析event信息
	ier, err := chain.DecodeEventRecords(c.Meta, result.(string), c.ChainName)
	if err != nil {
